...
class PaillierFederatedExecutorValue(executor_value_base.ExecutorValue):
  ...
  async def compute(self):
    if isinstance(self._value, executor_value_base.ExecutorValue):
      ...
      gathered_values = await asyncio.gather(*[
          PaillierFederatedExecutorValue(v, t).compute()
...
class PaillierFederatedExecutor(executor_base.Executor):
  ...
  async def _compute_intrinsic_federated_broadcast(self, arg):
    ...
    if len(arg.internal_representation) != 1:
      raise ValueError('Cannot broadcast a with a non-singleton representation.')
  ...
  async def _compute_intrinsic_federated_reduce(self, arg):
    py_typecheck.check_type(arg.type_signature, computation_types.NamedTupleType)
    py_typecheck.check_type(arg.internal_representation, anonymous_tuple.AnonymousTuple)
