...
TFE is about generating tfe.Computations, the same way that TF can be thought of as being about generating tf.SavedModels and TFF about generating tff.FederatedComputations.
...
```python
# players involved in definitions
prediction_client = tfe.Player('prediction_client')
```
...
Class hierarchy with `tfe.tensors.Tensor` (also exposed as `tfe.Tensor`) at the root:

- `tfe.tensors.EncryptedTensor`
  - `tfe.tensors.AdditiveTensor`
    - `tfe.tensors.TwoPlayerAdditiveTensor`
...
# we also want some values to be processed in plaintext;
# this protocol will replicate and process them on both servers
# to avoid network overhead
plaintext_protocol = tfe.protocols.Plaintext({server0, server1})
```
